...
if (isset($_POST['kata_kunci'])) {
	...
	$sql="select * from dosen_kelas where id like '%".$kata_kunci."%' or nip_dosen like '%".$kata_kunci."%' or kd_kelas like '%".$kata_kunci."%' order by id asc";

}else {
	$sql="select * from dosen_kelas order by id asc";
}
...
<head>
	...
	<body>
		...
		<form action="<?php echo $_SERVER["PHP_SELF"];?>" method="post">
			...
			<div class="p col-6 text-center">
				<a href="tambah_dosenkelas.php">Tambah Kelas Dosen</a>
			</div>
		</div>
		<table>
			<tr>
				...
				<tr>
					...
					<td>
						...
						<td>
							<?= $data->nip_dosen ?></td>
							<td>
								<a href="edit_dosenkelas.php?id=<?= $data->id ?>">Ubah</a> |
								<a href="hapus_dosenkelas.php?id=<?= $data->id ?>"onclick="javascript: return confirm('Anda yakin hapus ?')">Hapus</a>
							</td>
						</tr>
					<?php } ?>
